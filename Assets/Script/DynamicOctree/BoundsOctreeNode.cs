...
public class BoundsOctreeNode<T> {
    ...
    public bool IsContaining(ref Bounds checkBounds)
    {
        if (!Encapsulates(bounds, checkBounds))
            return false;

        // Check against any objects in this node
        for (int i = 0; i < objects.Count; i++)
        {
            if (Encapsulates(objects[i].Bounds, checkBounds))
                return true;
        }
        ...
        if (children != null)
        {
            for (int i = 0; i < 8; i++)
            {
                if (children[i].IsContaining(ref checkBounds))
                    return true;
            }
        }
    ...
    public void GetContainedBy(ref Bounds checkBounds, List<T> result)
    {
        // Check against any objects in this node
        for (int i = 0; i < objects.Count; i++)
        {
            if (Encapsulates(checkBounds, objects[i].Bounds))
            {
        ...
        if (children != null)
        {
            int bestFit = BestFitChild(checkBounds);
            children[bestFit].GetContainedBy(ref checkBounds, result);
        }
    }
